...
var testRoot = Path.Combine(Path.GetTempPath(), $"orbitalvue-feature-probe-{Guid.NewGuid():N}");
...
try
{
    ...
    try
    {
        var protectedLegacyCredentials = ProtectedData.Protect(
            ...
            Encoding.UTF8.GetBytes("OrbitalVue.XtreamCredentials.v1"),
    ...
    if (OperatingSystem.IsWindows())
    {
        ...
        var instanceScope = $"OrbitalVue.FeatureProbe.{Guid.NewGuid():N}";
        using var primaryInstance = new OrbitalVueSingleInstance(waitForPreviousInstance: false, scope: instanceScope);
        if (!primaryInstance.IsPrimary)
            throw new InvalidOperationException("The first OrbitalVue process did not acquire the single-instance coordinator.");
        ...
        await Task.Run(() =>
        {
            using var secondaryInstance = new OrbitalVueSingleInstance(waitForPreviousInstance: false, scope: instanceScope);
            if (secondaryInstance.IsPrimary)
                throw new InvalidOperationException("A second OrbitalVue process acquired the active recorder instance.");
    ...
    var maintenance = new OrbitalVueMaintenanceService(testRoot);
    ...
    var backupPath = Path.Combine(testRoot, "probe.orbitalvue-backup");
    ...
    if (backupCount != 3 + managedSourceCacheSnapshots.Count || !File.Exists(backupPath))
        throw new InvalidOperationException("OrbitalVue backup creation did not capture the expected protected data.");
    ...
    if (restoredCount != 3 + managedSourceCacheSnapshots.Count || restoredSettings.LastChannelKey != first.StableKey ||
        ...
        !File.Exists(Path.Combine(testRoot, "before-last-restore.orbitalvue-backup")))
        throw new InvalidOperationException("OrbitalVue backup restore or automatic rollback protection failed.");

    var legacyBackupPath = Path.Combine(testRoot, "legacy-3.6.orbitalvue-backup");
    ...
    var legacyRestoreCount = await new OrbitalVueMaintenanceService(legacyRestoreRoot).RestoreBackupAsync(legacyBackupPath);
    ...
    if (legacyRestoreCount != 1 || restoredLegacySettings.LastSource != "https://legacy-backup.invalid/list.m3u")
        throw new InvalidOperationException("A OrbitalVue 3.6 backup could not be restored by 3.7.");
    ...
    await maintenance.ExportDiagnosticsAsync(
        diagnosticPath,
        diagnosticSettings,
        new OrbitalVueDiagnosticContext(2, 1, "url", false, 2, first.StableKey, null));
    ...
    Console.WriteLine("OrbitalVue 3.6 backup restore compatibility: PASS");
...
{
    Console.Error.WriteLine($"OrbitalVue feature verification: FAIL - {exception.Message}");
    return 1;
}
finally
{
...
static async Task RunMicrosoftStorePremiumSelfTestAsync()
{
    ...
    const string productId = "com.orbitalvue.personal-media-centers";
...
static async Task RunMediaCenterSelfTestAsync(string testRoot)
{
    ...
    var service = new MediaCenterSourceService(credentialStore, http, "orbitalvue-win-feature-probe");
    var lockedArtworkService = new MediaCenterSourceService(
        credentialStore,
        http,
        "orbitalvue-win-locked-artwork-probe",
        PremiumAccessPolicy.Evaluate("store", hasVerifiedStorePurchase: false));
    ...
    try
    {
        await service.LoadArtworkAsync("orbitalvue-artwork://plex/emby-server-1/200", 320);
    ...
    string embyToken)
{
    ...
    var lockedService = new MediaCenterSourceService(
        ...
        new HttpClient(lockedHandler),
        "orbitalvue-locked-plex-account-probe",
        PremiumAccessPolicy.Evaluate("store", hasVerifiedStorePurchase: false),
    ...
    if (identityText.Contains("Ed25519", StringComparison.Ordinal) ||
        identityText.Contains("orbitalvue-plex-account-probe", StringComparison.Ordinal) ||
    ...
    var mismatchService = new MediaCenterSourceService(
        ...
        "orbitalvue-mismatched-plex-account-probe",
    ...
    var revocationService = new MediaCenterSourceService(
        ...
        "orbitalvue-revoked-plex-account-probe",
        premiumAccessProvider: () => runtimeAccess,
...
{
    var root = Path.Combine(Path.GetTempPath(), $"orbitalvue-dvr-probe-{Guid.NewGuid():N}");
