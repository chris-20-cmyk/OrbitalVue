...
public sealed class PlexDeviceIdentityStore
{
    ...
    private static readonly byte[] Entropy = Encoding.UTF8.GetBytes("StreamVue.PlexDeviceIdentity.v1");
    private readonly string _identityPath;
    private readonly object _gate = new();

    public PlexDeviceIdentityStore(string? identityPath = null)
    {
        _identityPath = Path.GetFullPath(identityPath ?? StreamVueDataPaths.Resolve("plex-device-identity.v1.bin"));
    }
