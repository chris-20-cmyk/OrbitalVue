...
public sealed partial class MediaCenterSourceService
{
    ...
    private const string ClientVersion = "5.7.0";
    ...
    {
        var path = OrbitalVueDataPaths.Resolve("media-center-device-id.v1.txt");
        try
        {
            if (File.Exists(path))
            {
            ...
            var generated = $"orbitalvue-win-{Guid.NewGuid():N}";
