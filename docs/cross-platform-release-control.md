...
This check is deliberately independent of signing credentials. It reads only committed public configuration and source metadata, including the fail-closed technical privacy inventory and Store-listing/asset contract.
...
See [privacy and Store disclosures](privacy-and-store-disclosures.md), [Store listing production](store-listing-production.md), [premium entitlements](premium-entitlements.md), [distribution and signing choices](distribution-and-signing.md), and each platform README for the detailed implementation boundary.
