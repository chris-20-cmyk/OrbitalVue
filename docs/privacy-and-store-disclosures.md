...
- Apple purchase verification remains within StoreKit. A future Android verifier would receive package name, product ID, and purchase token. A future Samsung verifier would receive the checkout customer ID, service country, app ID, and product ID. Those future verifier routes remain unconfigured and Store-locked.
...
| Samsung Seller Office | Review the privacy disclosure plus Samsung Checkout identity and future verifier behavior, then document retention/deletion. |
