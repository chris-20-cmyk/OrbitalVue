...
- Apple purchase verification remains within StoreKit. The implemented but undeployed Android verifier receives package name, product ID, and a transient purchase token. The implemented but undeployed Samsung verifier receives the Checkout customer ID, service country, app ID, and product ID. The shared service does not log request bodies or return those identifiers, but its production host, retention policy, Store disclosures, and real-device evidence remain unconfigured and Store-locked.
