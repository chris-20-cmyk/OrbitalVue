...
- Apple Plex account discovery sends a stable public Ed25519 device-registration key to Plex's signed-PIN endpoint. The matching private key and random client identifier stay in Keychain and may remain after app deletion when retained by the operating system. The returned Plex account token exists only in core memory while StreamVue verifies the account and fetches available servers; SwiftUI receives no token, and only the selected server-scoped token is retained in an origin-bound Keychain record after the server identity matches the selected resource.
