...
- Native one-time purchase foundations: StoreKit 2 on iOS/tvOS, Google Play Billing 9.1.0 on Android/Google TV, a Microsoft Store durable-license adapter on Windows, and Samsung Checkout with a protected DPI verifier, including localized pricing, Buy/Restore actions, live entitlement changes, and fail-closed verification. The shared server package verifies Google ProductPurchaseV2 state and Samsung product/history HMACs without returning tokens or account identifiers. LG is explicitly unavailable until its required third-party billing route is contracted and verified.
