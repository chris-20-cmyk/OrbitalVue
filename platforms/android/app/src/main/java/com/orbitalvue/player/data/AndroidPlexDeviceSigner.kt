...
internal class AndroidPlexDeviceSigner(
    ...
    private fun verifyMasterKey(masterAead: Aead) {
        val probe = "orbitalvue-plex-keystore-check".toByteArray(Charsets.UTF_8)
    ...
    private companion object {
        val KEYSET_LOCK = Any()
        val KEYSET_ASSOCIATED_DATA = "orbitalvue:plex-device-signing:v1".toByteArray(Charsets.UTF_8)
        const val PREFERENCES = "orbitalvue-plex-device-signing-v1"
        const val KEYSET_NAME = "plex_ed25519_keyset"
        const val MASTER_KEY_ALIAS = "com.orbitalvue.player.plex-signing.v1"
    }
}
