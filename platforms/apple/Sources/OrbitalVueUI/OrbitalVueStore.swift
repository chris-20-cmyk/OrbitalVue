...
@MainActor
@Observable
public final class OrbitalVueStore {
    public private(set) var isLoading = true
    public private(set) var loadingLabel = "Preparing your library…"
    public private(set) var catalog: OrbitalVueCatalog?
    ...
    public func playbackChannel(for channel: CatalogChannel) async -> ResolvedPlaybackChannel? {
        guard URL(string: channel.stream.uri)?.scheme?.lowercased() == "orbitalvue-media" else {
    ...
    public static func preview() -> OrbitalVueStore {
        let suite = UserDefaults(suiteName: "OrbitalVuePreview-\(UUID().uuidString)")!
        let store = OrbitalVueStore(defaults: suite)
