...
public enum MediaLibraryBrowsePolicy {
    ...
    public static func parseDate(_ value: String) -> Date? {
        ISO8601DateFormatter.streamVueDate(from: value)
    }
}

public extension CatalogChannel {
    var isMediaCenterItem: Bool {
        tags?.contains("media-center") == true || ["streamvue-media"].contains(URL(string: stream.uri)?.scheme?.lowercased() ?? "")
    }
...
extension ISO8601DateFormatter {
    static func streamVueDate(from value: String) -> Date? {
    ...
    static func streamVueString(from date: Date) -> String {
        let formatter = ISO8601DateFormatter()
        formatter.formatOptions = [.withInternetDateTime, .withFractionalSeconds]
