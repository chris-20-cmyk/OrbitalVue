...
struct MediaCenterTests {
    ...
    func signedPlexAccountDiscovery() async throws {
            ...
            case ("clients.plex.tv", "/api/v2/pins/42"):
                ...
                if segments.count == 3,
                   ...
                   let claims = try? JSONSerialization.jsonObject(with: claimsData) as? [String: Any] {
                    ...
                    #expect(claims["iss"] as? String == "orbitalvue-test-device")
        ...
        #expect(challenge.authorizationURL.fragment?.contains("clientID=orbitalvue-test-device") == true)
    ...
    func connectsDiscoveredPlexServerSecurely() async throws {
        ...
        let service = MediaCenterService(
            httpClient: http,
            secretStore: secrets,
            plexClientIdentifier: "orbitalvue-test-device"
        )
    ...
    func persistsMediaCenterSource() async throws {
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("OrbitalVueMediaCenterTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let service = MediaCenterService(
            httpClient: http,
            secretStore: MediaCenterMemorySecretStore(),
            plexClientIdentifier: "orbitalvue-repository-test"
        )
    ...
    func mapsEmbySecurely() async throws {
        ...
        #expect(catalog.channels.first?.stream.uri == "orbitalvue-media://emby/emby-server-1/item-1")
